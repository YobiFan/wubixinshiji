...
2025-10-26更新：
- 修复了几个一级简码编码错误
- 词库优化，由于wubilex词库太大，改为以广受好评的极点词库为主，Rime86五笔和wubilex为辅，如果想要wubilex的大词库，可在`wubixinshiji.dict.yaml`文件中取消`wubi_newage_dicts/ext-wubilex_other`前面的“#”。
- 把袖珍简化字拼音直接搬过来了，原因是袖珍简化字拼音是拼音反查的手段，而它的词库太小，并不包含大部分本方案的词语，我将本方案的的词语加入的反查的词库中，只是目前有个小问题，反查时部分词语会重复，待我闲了看看要不要解决下，不严重，只是不完美。
...
- 本方案中录入了一些**没有汉字**的词条，也就是**由英文字母、数字和符号组成**的词。如`MP3，iPhone 15，Video CD`等等。这部分的编码是较为特殊的，而且**不参与自动造词**，具体规则如下：
  1. 忽略符号，如`空格 + - * /`等等
  2. 大写转小写
  3. 数字部分用阿拉伯数字`0-9`用汉字`〇-九`的新世纪五笔编码的第一码代替。
  4. 比如`MP3`的编码是 `mpd`， `iPhone 5S`的编码是`iphonegs`，`C++`的编码是`c`，`AI`的编码是`ai`。
